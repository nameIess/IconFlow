...
- **Global config** — Works as `npm i -g mac-icns-cli` with config in `~/.config/mac-icns/`
...
**Options:** `--index <n>`, `--limit <n>`, `--page <n>`, `--out <dir>`, `--all`, `--dry-run`, `--delete-icns`, `--verbose`, `--format icns|ico`
